...
한자 읽기·부수·획순 데이터 등 제3자 자료의 출처와 라이선스는 `THIRD_PARTY_NOTICES.md`를 따릅니다. 특히 `public/data/hanzi-stroke-glyphs-v1.json`은 Arphic Public License를 상속한 자료이므로, 배포본에는 해당 라이선스 고지가 함께 가야 합니다. 이 파일은 선택 항목(획순 안내, 기본 꺼짐)을 켠 사람에게만 내려갑니다.
