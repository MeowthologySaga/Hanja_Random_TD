 ...
 *  - `idiom-order-seal-1~4-v1.png`: 60x60 원본을 20x20 으로 줄여 쓰는 순번 인장(RGBA).
 *  - `idiom-seal-ripple-ring-200-v1.png`: 200x200 white-alpha 마스크. 성어 색으로 tint 한다.
 *
...
export function idiomOrderSealImage(order: IdiomOrder): HTMLImageElement {
  return imageFor(`idiom-order-seal-${order}-v1.png`);
}

export function idiomRippleRingImage(): HTMLImageElement {
  return imageFor("idiom-seal-ripple-ring-200-v1.png");
}
