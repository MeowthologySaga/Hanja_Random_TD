...
/**
 * 조기 출전 보너스가 값을 매기는 시간(초).
 *
 * 셈은 예전 그대로 「남은 준비 시간 ÷ 2」다. 달라진 것은 준비 시계가 8초에서
 * 11초로 길어졌다는 것뿐인데, 그 3초를 그냥 두면 보너스도 함께 커진다 —
 * 자동 판정은 매 웨이브 곧바로 [지금 시작]을 눌러 늘어난 상한을 온전히
 * 챙긴다. 그래서 준비만 11초로 늘렸을 때 135판 승률이 0.556 에서 0.63 으로
 * 뛰었다(밴드 상한 0.60). 웨이브당 +1엽전만 얹어도 0.593 까지 오르는 것을
 * 따로 재어 원인을 못 박았다.
 *
 * 그래서 값이 매겨지는 시간을 예전 그대로 8초에 묶는다. 늘어난 3초는 준비의
 * **꼬리**에 붙어 값이 없다 — 그 무렵이면 보너스는 이미 0 이다. 부적을 쓰라고
 * 준 시간이니 값을 매기지 않는 것이 맞고, 덕분에 시계를 만져도 경제가 안
 * 흔들린다. 어느 순간에 눌러도 예전과 같은 액수가 나온다.
 */
export const EARLY_START_PAID_SECONDS = 8;

/** 보스 앞뒤의 긴 준비도 같은 이유로 예전 시계(12초)에 값을 묶어 둔다. */
export const EARLY_START_BOSS_PAID_SECONDS = 12;

/**
 * 부활 부적이 우두머리전에서 되돌려주는 시간(초).
 *
 * ③ 이후로 우두머리 시계는 문턱이라 중간 판에서는 지지 않는다 — 이 값이
 * 실제로 쓰이는 자리는 **마지막 우두머리** 하나다. 30초는 한 바퀴를 더 도는
 * 시간이 아니라 "지금 붙어 있는 화력으로 마저 깎을" 시간이다.
 */
export const REVIVAL_BOSS_SECONDS = 30;
 ...
 * v039 재보정: 3.62 → 3.30.
 *
 * 광역 사다리(AOE_RANK_WINDOW)가 캐주얼의 저별 장판을 통째로 걷고 3성 이후도
 * 반경을 1/3 에서 시작시켰다 — 캐주얼은 적 체력이 표준의 세 배가 넘어 장판에
 * 크게 기대므로, 45런 승률이 0.622 에서 **0.333** 으로 떨어졌다(밴드 하한 0.45).
 *
 * 줄어든 것은 「닿는 넓이」이므로 되돌려 줄 것은 「버텨야 할 총량」이다. 짝시드
 * 45런으로 셋을 견줬다 — 3.40 은 0.378(여전히 미달) · 3.30 은 0.556 · 3.20 은
 * 0.600. 밴드 한가운데인 3.30 을 잡는다(45런 잡음이 ±15pp 라 가운데가 안전하다).
 */
const MODE_ENEMY_HP_SCALE: Record<GameMode, number> = { standard: 1, casual: 3.30 };
 ...
 */
/**
 * 광역(장판·확산)의 **별 사다리** — v039.
 *
 * "낮은별부터 광역기 효과가 범위가 너무커 별별로 단계별로 커져야하고 …
 *  최소 3성부터 광역기 넣고, 가장 작은 범위는 현재의 1/3까지 줄여"(사용자).
 *
 * 여태 광역은 **1성부터 거의 다 컸다.** 오행 장판이 105~125 에 별당 +5,
 * 서리길·진흙밭 110, 잔화 95 — 1성 자령이 반경 110 짜리 원을 깔았고, 별을
 * 올려도 20% 남짓밖에 안 커져 「별을 올린 보람」이 반경으로는 안 보였다.
 *
 * 그래서 광역을 **별로 여는 문**으로 바꾼다.
 *  · `open` 미만 — 광역이 아예 없다. 그 자령의 기술은 한 체만 때린다.
 *  · `open` 에서 지금 반경의 1/3, `full` 에서 지금 반경(별 보너스 없이).
 *
 * 그래서 오늘의 반경은 **이제 8성의 반경**이고, 오늘 가장 작았던 원(잔화 95 ·
 * 화행 장판 105)은 중후반 별에서야 만난다.
 *
 * 이 사다리는 장판(오행·서리길·진흙밭·잔화)과 확산(화행 폭발·역할 확산·밀집
 * 구간) **전부**에 같이 걸린다 — 한 곳만 줄이면 "가장 작은 범위"가 딴 데
 * 남아 규칙이 깨진다.
 */
export const AOE_RANK_WINDOW = { open: 3, full: 8 } as const;

/** 문이 막 열린 자리의 반경 배율 — 「현재의 1/3」. */
export const AOE_MIN_RADIUS_SCALE = 1 / 3;

/**
 * 이 진급의 광역 반경 배율. **null 이면 광역이 아직 안 열렸다** — 부르는 쪽은
 * 장판을 깔지 말고 확산도 돌리지 말아야 한다(0 을 돌려주면 반경 0 짜리 장판이
 * 화면에 점으로 남는다).
 */
export function aoeRadiusScale(rank: number, mode: GameMode): number | null {
  /*
   * 표준(자형연성)은 이 사다리를 타지 않는다.
   *
   * 사용자가 말한 축은 **별**이고, 별은 캐주얼(별승급)에만 있다. 표준의 단계
   * (1~5)는 「몇 번 조합했나」라서 별과 눈금이 다르다 — 같은 창을 단계에 씌워
   * 재어 봤더니 1단계 자령이 광역을 통째로 잃어 표준 135런 승률이 0.659 에서
   * **0.000** 으로 무너졌다(실측). 축이 다르면 사다리도 따로 놓아야 한다.
   */
  if (mode !== "casual") return 1;
  if (rank < AOE_RANK_WINDOW.open) return null;
  const span = Math.max(1, AOE_RANK_WINDOW.full - AOE_RANK_WINDOW.open);
  const step = Math.max(0, Math.min(1, (rank - AOE_RANK_WINDOW.open) / span));
  return AOE_MIN_RADIUS_SCALE + (1 - AOE_MIN_RADIUS_SCALE) * step;
}
