 ...
 * 부적 기대 수입(상한 3회/웨이브 · 성공률 100% 가정):
 *   엽전 60% × 평균 10  = 6.0/회
 *   문기 30% × 1        = 0.3 문기/회
 *   무료권 10% × 1      = 0.1 소환권/회 (기본 소환가 7~24 엽전 상당)
 *   → 3회면 웨이브당 18 엽전 + 0.9 문기 + 0.3 소환권 ≈ 25 엽전 상당.
 * 초반(1-10 웨이브) 총수입 24.3 과 맞먹는다 — 그대로 두면 초반 수입이 두 배다.
 *
 * 처방은 계수가 아니라 **장부**다. 부적 보상을 지급할 때 그 엽전 환산액을
 * 빚으로 적어 두고, 웨이브 정산(웨이브 완료 보상 + 은행 이자)에서 조금씩
 * 되갚는다. 계수 방식과 달리 "부적 만들기를 켜 두고 안 쓰는 사람"은 한 푼도
 * 손해 보지 않는다 — 안 쓰면 빚이 0 이라 정산이 통째로 그대로다(기본 켜짐
 * 결정의 전제). 시뮬 봇은 부적을 쓰지 않으므로 빚이 언제나 0 이고, 따라서
 * `--runs=45 --mode=casual` · `--runs=135` 수치는 이 변경 전과 완전히 같다.
 */

/**
 * 문기 1개의 상환 환산액. 문기는 엽전으로 살 수 없는 화폐라 정가가 없다 —
 * 실질 획득 경로(기본 소환 1회 ≈ 7~24 엽전이 1★ 하나 ≈ 문기 1~2)를 보수적으로
 * 접어 8 엽전으로 잡는다. 부적 쪽이 조금 싸게 사는 셈이고, 그 차이가 "따라
 * 쓰는 수고에 붙는 웃돈"이다.
 */
export const TALISMAN_ESSENCE_GOLD_VALUE = 8;

/**
 * 한 웨이브 정산에서 되갚을 수 있는 몫의 상한.
 *
 * 1 로 두면 초반 웨이브 보상이 통째로 사라져 "방어 성공 · 보상 0엽전"이 뜬다 —
 * 갚는 일이 벌처럼 보이면 안 되므로 정산의 6할까지만 물린다. 남은 빚은
 * 다음 웨이브로 넘어가 결국 전부 갚아진다(수입 총량은 대등, 시점만 앞당겨진다).
 */
export const TALISMAN_REBATE_WAVE_SHARE = 0.6;
