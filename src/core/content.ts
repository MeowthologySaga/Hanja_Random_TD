...
import type { EnemyArchetype, GameMode, Point, RunPhase, WavePlan, Wuxing } from "./types";
...
/**
 * 이어하기가 사람을 **어디에** 내려놓는가 (v042).
 *
 * 여태 화면은 늘 「N웨이브 준비 시간부터 이어서 봉인합니다」라고 말했다. 실측하면
 * 자동 저장 지점 1110개 가운데 **53.7%가 교전 중**이다(웨이브 시작 46.8% · 잔존 합류
 * 6.8%). 사람 기준 시간 가중으로는 약 **70%**가 교전이라, 그 문장은 열 번에 일곱 번
 * 거짓이었다.
 *
 * 잔존 합류 저장은 되살리면 적이 **중앙 17체 · 최대 54체** 이미 서 있는 전장이고,
 * 그중에는 노선을 한 바퀴 돈 적도 있다. 그런 자리에 「준비 시간부터」라고 말하면
 * 손이 준비되기 전에 판이 굴러간다 — 복원 뒤 첫 적이 서기까지 준비 저장은 11.02초,
 * 교전 저장은 **0.017초(한 프레임)**다.
 *
 * 문장은 코어가 만든다(v041 우두머리 시계 · v042 waveClearNotice 선례). 화면이 phase
 * 로 갈라 문장을 갈아 끼우면 같은 사실이 자리마다 다른 말로 나간다.
 */
export function resumeNotice(wave: number, phase: RunPhase, enemies: number): string {
  if (phase !== "combat") return `${wave}웨이브 준비 시간부터 이어서 봉인합니다.`;
  if (enemies > 0) return `${wave}웨이브 교전 중부터 이어집니다 — 적 ${enemies}체가 이미 전장에 있습니다. 판은 멈춰 있습니다.`;
  return `${wave}웨이브 교전 시작부터 이어집니다. 판은 멈춰 있습니다.`;
}

/**
 * 종료 화면이 실을 글자 칩 수 (v042).
 *
 * 격자는 세 칸씩 네 줄이고 열한 칸이 차 있어 **넷째 줄에 162.7×57 슬롯 하나가
 * 비어 있다**(실측). 그 아래로도 208.6px 이 남는다. 그러니 자리를 새로 짜지 않고
 * 전폭 띠 한 장을 얹는다 — 카드 안쪽 폭 502px 에 24px 칩이면 열여덟은 든다.
 *
 * 열둘로 잡은 까닭은 여백이다. 판당 서로 다른 글자가 93.0자라 어차피 전부는 못 싣고,
 * 「몇 자를 만났는지」는 숫자가 말한다. 칩은 **되짚을 실마리**지 목록이 아니다.
 */
export const RUN_TRACE_CHIP_LIMIT = 12;

/**
 * 이 판이 만난 글자를 한 줄로 (v042).
 *
 * 실측이 이 문장을 세웠다. 한 판은 웨이브마다 야생 글자를 데려오고 그 글자가 적의
 * 몸에 찍혀 나온다 — 판당 **99.2자**, 서로 다른 **93.0자**. v042 가 배너로 읽어 주고
 * 소리로도 읽어 주게 됐는데, **판이 끝나면 그 99자가 통째로 증발했다.** 종료 화면의
 * `발견 한자` 는 소환·승급·합성만 세고 웨이브를 한 번도 안 지나므로(실측: 어느 칸에도
 * 안 나오는 서로 다른 글자 **75.8자**) 그 자리를 못 메운다.
 *
 * 숫자 열한 칸 중 승리 런에서 다섯은 매번 바이트까지 같은 글자다(처치한 적은 웨이브
 * 편성이 결정적이라 언제나 2795). 문제는 「숫자가 모자라다」가 아니라 「숫자만 있다」라,
 * 열두째 숫자를 붓는 대신 **이 판에만 있는 것**을 말한다.
 */
export function runTraceNotice(distinct: number, shown: number): string {
  /*
   * `distinct >= 1` 일 때만 부른다 — 한 자도 못 만난 판에서는 화면이 띠를 통째로
   * 숨긴다(dialogs/end.ts). 처음에는 여기에 「야생 글자를 만나지 못했습니다」를 적어
   * 뒀는데, **화면에 닿을 수 없는 문장**이었다. 아무도 못 볼 말을 코어가 만들고 시험이
   * 그것을 못 박고 있으면, 다음 사람은 그 갈래가 살아 있다고 믿는다.
   */
  if (shown >= distinct) return `이번 판에서 만난 글자 ${distinct}자`;
  return `이번 판에서 만난 글자 ${distinct}자 — 마지막 ${shown}자`;
}
