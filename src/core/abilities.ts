...
export const SEMANTIC_ABILITY_TABLE: Record<SemanticFamily, SemanticPattern> = {
  ...
  mountain: semanticPattern("mountain", "front", 7, 0.3, "산맥 진압", "山", "stun", "7번째 공격", "진행도 후퇴", "길을 울려 선두 적을 뒤로 밀고 잠시 발을 묶습니다.", "#d8ab74"),
  speech: semanticPattern("speech", "cluster", 6, 0.18, "언령 메아리", "言", "support", "6번째 공격", "주변 능력 가속", "같은 진의 자령에게 언령을 퍼뜨려 공격 대기를 줄입니다.", "#dda4ff"),
  motion: semanticPattern("motion", "fastest", 5, 0.27, "추행 봉쇄", "行", "control", "5번째 공격", "최고속 추적", "가장 빠른 적을 추적해 앞으로 나아간 거리를 되돌립니다.", "#7ee7d5"),
...
export function composeAbilityLoadout(input: AbilityComposeInput): AbilityLoadout {
  ...
  const tuning = {
    ...
    supportCooldown: input.role === "support" ? 0.18 + input.stage * 0.025 : 0,
