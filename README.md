...
소환은 별을 **가중**하는 것이 아니라 후보 풀 자체를 별 구간으로 자릅니다. 구간 밖의 별은 뽑기로 아예 나오지 않습니다. 별승급 진법 전용이며, 후보가 부족한 지역에서는 상품이 노출되지 않습니다.

| 상품 | 별 구간 | 기본가에 얹는 값 |
|---|---|---|
| 기본 · 탐색 · 중복 수집 | 1~3★ | +0 / +2 / +2엽전 |
| 중급 소환 | 2~5★ | +5엽전 |
| 고급 소환 | 3~8★ | +12엽전 |

구간 안에서도 낮은 별이 더 흔합니다(별 하나 오를 때마다 ×0.55). 상위 별은 뽑기가 아니라 3합으로 올리는 것이 기본 경로이고, 중급·고급 소환이 그 지름길입니다. 기본 소환가는 `7 + ⌊누적 소환 횟수 / 12⌋` 엽전이며 최대 24입니다. 10연 소환은 10웨이브에 열리고, 별승급 진법에서는 열 장 안에 기본 밴드 상단인 3★ 1기를 보장합니다.
