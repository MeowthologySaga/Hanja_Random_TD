...
| 상품 | 별 구간 | 기본가 배수 | 기본가 7 | 기본가 24 |
|---|---|---|---|---|
| 기본 소환 | 1~3★ | ×1 | 7엽전 | 24엽전 |
| 탐색 · 중복 수집 | 1~3★ | ×1.3 | 9엽전 | 31엽전 |
| 중급 소환 | 2~5★ | ×1.7 | 12엽전 | 41엽전 |
| 고급 소환 | 3~8★ | ×2.7 | 19엽전 | 65엽전 |

구간 안에서도 낮은 별이 더 흔합니다(별 하나 오를 때마다 ×0.55). 상위 별은 뽑기가 아니라 3합으로 올리는 것이 기본 경로이고, 중급·고급 소환이 그 지름길입니다. 기본 소환가는 `7 + ⌊누적 소환 횟수 / 12⌋` 엽전이며 최대 24입니다. 목적 소환의 값은 **정액이 아니라 기본가의 배수**입니다 — 기본가가 올라도 상품 사이의 값 비율이 그대로여서, 초반에 성립하던 "기본=화력 효율 · 티어=별 프리미엄" 구조가 후반까지 유지됩니다(계보 소환은 자형연성 전용, ×1.45). 10연 소환은 **웨이브 자물쇠 없이 언제나 열려 있고** 균형가 열 장 값을 그대로 받으며(첫 10연 70엽전), 별승급 진법에서는 열 장 안에 기본 밴드 상단인 3★ 1기를 보장합니다.
