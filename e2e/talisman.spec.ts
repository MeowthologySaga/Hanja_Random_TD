...
test("the default-on talisman tab turns a submitted trace into a jaryeong reward visit", async ({ page }) => {
   ...
   *
   * v035 ① 로 경제 밖 보상 셋(자령 강림·봉인의 손·문기의 숨)이 더 붙었고,
   * v041 이 앞의 둘을 **강림부 한 장**으로 합쳤다 — 즉시 터지던 것을 손에 쥐었다
   * 위급할 때 사르게 바꾼 것이다. 어느 갈래가 걸릴지는 굴림이 정하므로 전부
   * 받는다. 여기서 지키는 것은 「무엇을 얼마나」가 **글자로 읽힌다**는 것이지
   * 갈래가 아니다.
   */
  await expect(page.locator(".talisman-gift > em"))
    .toHaveText(/^((엽전|[金木水火土] 문기|무료 소환권) \+\d+|강림부 \+1 · \d\/3장|강림부 가득 · [金木水火土] 문기 \+\d+|(자령 강림|봉인의 손) · \d+체 (타격|묶음)|문기의 숨 · 준비 \+\d+초)$/);
